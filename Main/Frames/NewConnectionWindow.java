...
public class NewConnectionWindow extends JFrame {
    ...
    private void startButtonClicked(MouseEvent e) {
        if(getConnectto().isSelected()){
            try {
                Main.startClient(getIpaddr().getText(), Integer.parseInt(getTcpportonhost().getText()), Integer.parseInt(getUdpportonhost().getText()));
            }catch(Exception ex){
    ...
    public JTextField getTcpportonhost() {
        return tcpportonhost;
    }

    public JTextField getUdpportonhost() {
        return udpportonhost;
    }
